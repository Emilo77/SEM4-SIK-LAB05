...
size_t check_file_size(char *filename) {
...
inline static uint16_t read_port(char *string) {
  ...
  PRINT_ERRNO();
  if (port > UINT16_MAX) {
    fatal(1, "%ul is not a valid port number", port);
  }
...
inline static void send_message(int socket_fd, void *message, size_t length,
                                int flags) {
  errno = 0;
